...
var create = module.exports = function(options){
	...
	var httpServer,
		...
		captureHostname = captureHostArr[0],
		caturePort = captureHostArr[1] || 80,
		expressServer = express(),
		captureUrl = "http://" + (captureHostname || utils.getExternalIpAddress()) + ":" + caturePort + "/";
	...
	if(captureHostname === "*") captureHostname = void 0;
...
function getApi(queen){
	...
	api.log = queen.log.bind(queen);
	api.debug = queen.debug.bind(queen);
...
Queen.prototype.proxyWorkforceResource = function(request, response){
	...
	if(workforce === void 0){
		...
		this.debug("[Queen] Can't fulfill proxy require, workforce has no run host: " + workforceId + "\n");
		response.send(404);	
	} else {
