...
var create = module.exports = function(options){
	...
	var socket,
		...
		host = options.host || getExternalIpAddress(),
		expressServer = express(),
		webRoot = STATIC_DIR,
		capturePath = "/capture.html",
		captureUrl = "http://" + host + ":" + port + capturePath,
		httpServer;
...
Queen.prototype.proxyWorkforceResource = function(request, response){
	...
	if(workforce === void 0){
		this.log("Couldn't find requested workforce: " + workforceId);
		response.send(404);	
	} else if(workforce.runUrl === void 0){
		this.log("Workforce has no run host: " + workforceId);
		response.send(404);	
	} else {
